public class ChessBoard {
    public ChessPiece[][] board = new ChessPiece[8][8]; // Игровое поле
    String nowPlayer;
    ...
    public void printBoard() {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (board[i][j] != null) {
                    System.out.print(board[i][j].getClass().getSimpleName().charAt(0) + " "); // Выводим первую букву класса фигуры
                } else {
    ...
    // Метод для перемещения фигур с учетом флага check
    public boolean moveToPosition(int startLine, int startColumn, int endLine, int endColumn) {
        if (checkPos(startLine) && checkPos(startColumn)) {
            ...
            if (board[startLine][startColumn].canMoveToPosition(this, startLine, startColumn, endLine, endColumn)) {
                ...
                board[startLine][startColumn] = null; // освобождаем старую клетку
    ...
    // Метод для проверки, что позиция находится на доске
